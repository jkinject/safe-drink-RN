...
- **캐릭터 에셋(assets/images/character/, 10장)은 배경 투명 RGBA 1024px.** 재생성 파이프라인: ① Codex 이미지 도구로 원본을 참고 이미지로 붙여 **순수 초록(#00FF00) 단색 배경**에 다시 그린다 — `codex exec -i ref.png -- "프롬프트"` 형식(`-i` 는 여러 파일을 먹으므로 프롬프트 앞에 `--` 필수), 투명 출력은 지원하지 않는다. 떠 있는 거품·하트·반짝이를 그대로 그리라고 명시할 것. ② `python3 scripts/key-green-characters.py in.png out.png [--height 1024]` 로 크로마키(Pillow 만 사용; 순수 초록은 어디든 배경, 가장자리 소프트 알파 + 초록 기여분 되돌리기 + 번짐 깎기). ML 배경 제거(Replicate background-remover)는 떠 있는 거품·하트를 지워 버려서 쓰지 않는다. 색 거리로 앱 배경색을 빼는 방식은 머리카락이 지저분해 폐기. Live Activity 용(`assets/liveActivity/`)은 같은 스크립트를 `--height 320` 으로.
