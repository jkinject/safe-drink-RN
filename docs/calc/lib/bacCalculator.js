 ...
 */
function accumulateUntil(finished, contribution, untilMs) {
    const sorted = [...finished].sort((a, b) => a.finishedAt - b.finishedAt);
    let bac = 0;
    let atMs = 0;
    let started = false;
    for (const record of sorted) {
        if (record.finishedAt > untilMs)
            break;
        if (started) {
            const decayed = bac - BETA * ((record.finishedAt - atMs) / 3600000);
 ...
 */
function soberAtMsOf(finished, contribution) {
    const acc = accumulateUntil(finished, contribution, Infinity);
    if (acc == null || acc.bac <= 0)
        return null;
    return acc.atMs + Math.round((acc.bac / BETA) * 3600000);
}
/**
 * birthYear 로부터 나이 계산.
 ...
 */
function resolveAge(profile, referenceYear) {
 ...
 */
export function calcR(profile, referenceYear) {
 ...
 */
export function calcBacContribution(record, profile, referenceYear) {
    const alcoholGrams = record.volumeMl * (record.abvPercent / 100) * DENSITY_ETHANOL;
 ...
 */
export function currentBac(records, profile, nowMs) {
    const finished = finishedOnly(records);
    if (finished.length === 0)
        return 0;
    const nowYear = new Date(nowMs).getFullYear();
    return bacAtMs(finished, r => calcBacContribution(r, profile, nowYear), clampNowMs(finished, nowMs));
}
/**
 ...
 */
export function bacAt(records, profile, atMs) {
    const finished = finishedOnly(records);
    if (finished.length === 0)
        return 0;
 ...
 * @param referenceYear 나이 계산 기준 연도 (테스트 주입용). null 이면 자동 계산.
 */
export function estimatedSoberAt(records, profile, referenceYear) {
    const finished = finishedOnly(records);
    if (finished.length === 0)
        return null;
    return soberAtMsOf(finished, r => calcBacContribution(r, profile, referenceYear));
}
/** 현재 시점 기준 남은 분해 시간 (분, 올림). BAC 이 0 이면 0 반환. */
export function remainingMinutesCeil(records, profile, nowMs) {
...
export function remainingHours(records, profile, nowMs) {
    const bac = currentBac(records, profile, nowMs);
    if (bac <= 0)
        return 0;
    return bac / BETA;
...
export function totalAlcoholGrams(records) {
    return finishedOnly(records).reduce((sum, r) => sum + r.volumeMl * (r.abvPercent / 100) * DENSITY_ETHANOL, 0);
}
/**
 ...
 * @param referenceYear 나이 계산 기준 연도 (테스트 주입용). null 이면 자동 계산.
 */
export function totalBacContribution(records, profile, referenceYear) {
    return finishedOnly(records).reduce((sum, r) => sum + calcBacContribution(r, profile, referenceYear), 0);
}
// ── 표준 Widmark (성별 상수 r) 경로 ─────────────────────────────────────────
/** 성별 상수 r — 표준 Widmark 방식 (남 0.68 / 여 0.55) */
export function constantR(profile) {
    return profile.sex === 'male' ? 0.68 : 0.55;
}
/** 단일 기록의 BAC 기여 — 표준 Widmark 방식 (성별 상수 r 사용) */
export function calcBacContributionWithConstantR(record, profile) {
    const alcoholGrams = record.volumeMl * (record.abvPercent / 100) * DENSITY_ETHANOL;
 ...
 * finishedAt == undefined 인 기록은 제외한다.
 */
export function currentBacWithConstantR(records, profile, nowMs) {
    const finished = finishedOnly(records);
    if (finished.length === 0)
        return 0;
    return bacAtMs(finished, r => calcBacContributionWithConstantR(r, profile), clampNowMs(finished, nowMs));
}
/**
 ...
 * finishedAt == undefined 인 기록은 제외한다.
 */
export function estimatedSoberAtWithConstantR(records, profile) {
    const finished = finishedOnly(records);
    if (finished.length === 0)
        return null;
    return soberAtMsOf(finished, r => calcBacContributionWithConstantR(r, profile));
}
/** 남은 분해 시간 (시간 단위, double) — 표준 Widmark 방식 */
export function remainingHoursWithConstantR(records, profile, nowMs) {
 ...
 */
export function bacCurve(records, profile, samples = 60) {
    const finished = finishedOnly(records);
    if (finished.length === 0 || samples < 2)
        return [];
    ...
    if (totalMs <= 0)
        return [];
    const contribution = r => calcBacContribution(r, profile, firstYear);
