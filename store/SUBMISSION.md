...
| 5 (1.2.0) | `48966f5` | 2026-09-09 EAS 빌드(AdMob 배너·광고 제거 IAP·영어 프리셋). 내부 테스트 게시 → `remove_ads` 등록 → **같은 날 프로덕션 100% 승급·검토 제출**(en-US 등록정보·광고 선언·데이터 안전·광고 ID·콘텐츠 등급과 함께). AdMob 게재 심사 통과됨 |
| 6 (1.2.1) | `48cbe54` | 2026-09-11 EAS 빌드(인앱 리뷰 API 칭찬하기·술자리 공유·혈중알코올농도 표기). 프로덕션 100% 검토 제출. 같은 날 ko-KR ASO 등록정보(제목에 혈중알코올농도) 게시됨 |
| 7 (1.2.2) | `f3488c7` | 2026-09-11 EAS 빌드(술자리 그래프 이미지 공유 — view-shot+expo-sharing, 정점 기준 캐릭터, 앱 아이콘 푸터). aside(admin@epcompany.co.kr)로 프로덕션 100% 검토 제출, 출시 노트 ko/en. 경고는 R8 매핑 없음 하나뿐. **같은 날 12:36 게시됨**(Google Play에 제공됨). 이후 JS 수정 2건(영어 뱃지 줄바꿈·용어 통일)은 production OTA 로 게시. 2026-09-16 `a865845` OTA(캐릭터 10장 투명 교체; 번들에 iOS Live Activity JS 포함되나 Android 에선 비활성) |
